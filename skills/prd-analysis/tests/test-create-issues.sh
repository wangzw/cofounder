...
setup_fixture
run_with_stdin "$GOOD_JSON" "$CHECK" "$FIXTURE" "1"
run_with_stdin '{"issues":[{"criterion_id":"CR-X","file":"x.md","severity":"error","description":"another problem in x","suggested_fix":"fix it more concretely"}]}' "$CHECK" "$FIXTURE" "2"
...
run_with_stdin '{"issues":[{"criterion_id":"CR-X","file":"x.md","severity":"error","description":"same problem here","suggested_fix":"fix it concretely"}]}' "$CHECK" "$FIXTURE" "1"
# Now in round 2, same signature should get recurrence_of: I-001
run_with_stdin '{"issues":[{"criterion_id":"CR-X","file":"x.md","severity":"error","description":"same problem here","suggested_fix":"fix it concretely"}]}' "$CHECK" "$FIXTURE" "2"
...
fix again
'
run_with_stdin '{"issues":[{"criterion_id":"CR-X","file":"x.md","severity":"error","description":"recurred yet again","suggested_fix":"fix more carefully","recurrence_of":"I-002"}]}' "$CHECK" "$FIXTURE" "3"
...
setup_fixture
run_with_stdin "$GOOD_JSON" "$CHECK" "$FIXTURE" "1" "--dry-run"
