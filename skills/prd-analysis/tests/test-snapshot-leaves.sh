...
run_command "$CHECK" "/nonexistent-prd-xyz" 1
[ "$LAST_EXIT" = "2" ] && _record_pass || _record_fail "expected 2"
...
test_case "manifest enumerates feature & journey leaves"
setup_fixture
write_file "README.md" "# X"
write_file "features/F-001-login.md" "# login"
write_file "journeys/J-001-onboarding.md" "# onboarding"
"$CHECK" "$FIXTURE" "1" >/dev/null 2>&1
manifest="$FIXTURE/.review/round-1/leaves-manifest.yml"
grep -q "features/F-001-login.md" "$manifest" && _record_pass || _record_fail "F-001 missing"
grep -q "journeys/J-001-onboarding.md" "$manifest" && _record_pass || _record_fail "J-001 missing"
teardown_fixture
