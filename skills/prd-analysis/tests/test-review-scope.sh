...
build_bundle() {
    local did="$1"
    write_file "README.md" "# PRD"
    write_file "journeys/J-001.md" "j1"
    write_file "journeys/J-002-onboard.md" "j2"
    write_file "features/F-001.md" "f1"
    write_file "features/F-002-login.md" "f2"
    write_file "architecture/data-model.md" "dm"
...
grep -q "README.md" "$M" && grep -q "journeys/J-001.md" "$M" \
    && grep -q "features/F-001.md" "$M" && grep -q "architecture/data-model.md" "$M" \
...
H1=$(grep "features/F-001.md" -A1 "$M" | tail -1)
H2=$(grep "features/F-001.md" -A1 "$M2" | tail -1)
...
build_bundle 1
"$SNAP" "$FIXTURE" 1 >/dev/null
echo "MODIFIED" > "$FIXTURE/features/F-001.md"
"$SNAP" "$FIXTURE" 2 >/dev/null
...
grep -q "features/F-001.md" "$S" && _record_pass \
...
build_bundle 1
"$SNAP" "$FIXTURE" 1 >/dev/null
write_file "features/F-003-new.md" "f3"
"$SNAP" "$FIXTURE" 2 >/dev/null
assert_exit 0 "$SCOPE" "$FIXTURE" 2
assert_stdout_contains "mode=incremental"
assert_stdout_contains "changed=1"
...
build_bundle 1
"$SNAP" "$FIXTURE" 1 >/dev/null
rm "$FIXTURE/features/F-002-login.md"
"$SNAP" "$FIXTURE" 2 >/dev/null
assert_exit 0 "$SCOPE" "$FIXTURE" 2
assert_stdout_contains "mode=incremental"
assert_stdout_contains "changed=1"
