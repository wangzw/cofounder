...
#
# Enforces CR-PP-MM01: every ```mermaid block in PRD leaves MUST satisfy
...
MERMAID_COMMENT_TOLERATED='# F-005

```mermaid
flowchart LR
  %% \n and (key: value) in a comment must not fire CR-PP-MM01
  A[Plain] --> B
```
'
...
# ============================================================
test_case "CR-PP-MM01 fires on \\n literal in flowchart label"
setup_fixture
write_file "README.md" "# stub"
write_file "features/F-001-newline.md" "$NEWLINE_LITERAL"
assert_exit 1 "$CHECK" "$FIXTURE"
assert_stdout_contains "CR-PP-MM01"
assert_stdout_contains "F-001-newline.md"
assert_stdout_contains "\\\\n"
teardown_fixture

# ============================================================
test_case "CR-PP-MM01 fires on unquoted leading-/ label"
setup_fixture
write_file "README.md" "# stub"
write_file "features/F-002-path.md" "$UNQUOTED_PATH"
assert_exit 1 "$CHECK" "$FIXTURE"
assert_stdout_contains "CR-PP-MM01"
assert_stdout_contains "F-002-path.md"
assert_stdout_contains "parallelogram"
teardown_fixture

# ============================================================
test_case "CR-PP-MM01 fires on : inside parens in stateDiagram-v2 transition"
setup_fixture
write_file "README.md" "# stub"
write_file "features/F-003-state.md" "$COLON_IN_PARENS"
assert_exit 1 "$CHECK" "$FIXTURE"
assert_stdout_contains "CR-PP-MM01"
assert_stdout_contains "F-003-state.md"
...
setup_fixture
write_file "README.md" "# stub"
write_file "features/F-004-url.md" "$URL_PATH_PRESERVED_BODY"
assert_exit 0 "$CHECK" "$FIXTURE"
assert_stdout_contains "PASS"
teardown_fixture
...
setup_fixture
write_file "README.md" "# stub"
write_file "features/F-005-comment.md" "$MERMAID_COMMENT_TOLERATED"
assert_exit 0 "$CHECK" "$FIXTURE"
assert_stdout_contains "PASS"
teardown_fixture
...
setup_fixture
write_file "README.md" "# stub"
write_file "features/F-001-a.md" "$NEWLINE_LITERAL"
write_file "features/F-002-b.md" "$UNQUOTED_PATH"
write_file "features/F-003-c.md" "$COLON_IN_PARENS"
run_command "$CHECK" "$FIXTURE"
[ "$LAST_EXIT" = "1" ] && _record_pass || _record_fail "expected 1 got $LAST_EXIT"
echo "$LAST_STDOUT" | grep -q "F-001-a.md" && _record_pass || _record_fail "missing F-001"
echo "$LAST_STDOUT" | grep -q "F-002-b.md" && _record_pass || _record_fail "missing F-002"
echo "$LAST_STDOUT" | grep -q "F-003-c.md" && _record_pass || _record_fail "missing F-003"
teardown_fixture
...
setup_fixture
write_file "README.md" "# stub"
write_file "features/F-001.md" "$NEWLINE_LITERAL"
run_command "$CHECK" "$FIXTURE"
out1="$LAST_STDOUT"
run_command "$CHECK" "$FIXTURE"
out2="$LAST_STDOUT"
