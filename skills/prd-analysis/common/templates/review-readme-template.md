...
|---|---|---|
| `input.md` | `prepare-input` (script) | Raw user prompt, written verbatim. Sub-agents (consultant, planner, writer) Read referenced `@path` files and Fetch URLs on demand using their own tools. |
| `input-meta.yml` | `prepare-input` (script) | `word_count`, `char_count`, `has_code_block`, `has_structured_lists`. Used by `glossary-probe.sh` to compute `sparse_input`. |
...
| `leaves-manifest.yml` | `scripts/snapshot-leaves.sh` (review/index.md Step 1.5) | sha256 of every PRD leaf at the start of the read phase. Survives compaction (final round only) and feeds the next round's incremental-scope diff. |
| `review-scope.yml` | `scripts/compute-review-scope.sh` (review/index.md Step 1.6) | `mode: full \| incremental`, `reason`, `changed_leaves[]`, `unchanged_leaves[]`, and (v1.4+) `category_clusters[]` — one entry per LLM-criterion category active this round, each with `category`, `criteria[]`, `leaves[]`. The orchestrator fans out one cross-reviewer per cluster; each reviewer applies only its cluster's CR-IDs and honors the `mode` partition. |
