...
> **Infrastructure carryover — applies to skill-forge meta-skills only, NOT to PRD output.**
> This criterion is a mandatory cross-skill carryover from `skill-forge/common/review-criteria.md`
> (per writer-subagent "Mandatory cross-skill carryovers" section). It governs generated `SKILL.md`
> files produced by skill-forge, not PRD artifact bundles. PRD writers and cross-reviewers MUST
> NOT apply this criterion to any prd-analysis leaf (`README.md`, `journeys/`, `features/`,
> `architecture/`). It is retained here so that if prd-analysis itself is ever re-generated by
> skill-forge, the gate travels with the bundle.
...
- id: CR-S15
  ...
  severity: error
  applies_to: ["SKILL.md"]
  applies_to_context: meta-skill-bundle
  carryover_note: "Infrastructure carryover from skill-forge. Applies only to generated SKILL.md files, not to PRD artifact leaves. prd-analysis writers and cross-reviewers MUST skip this criterion."
  conflicts_with: []
  priority: 1
  incremental_skip: per_file
  rationale: |
