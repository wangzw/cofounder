...
- **Frontend draft impact** — Does the change invalidate the existing frontend draft at the baseline's Frontend Implementation Path? If so, mark the affected feature(s) as needing draft re-generation (clear the Confirmed date in their Frontend Draft Reference); after the draft is updated and re-validated for experience, set the new Confirmed date. Impact is derived from the spec delta alone — **do not list or read files under the Frontend Implementation Path**. Which features have a confirmed draft is determined from each feature file's Frontend Draft Reference section, not from the filesystem. (Production hardening of the affected code is owned by system-design's Production Promotion Plan, not by revise.)
