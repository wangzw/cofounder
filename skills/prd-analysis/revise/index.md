...
them **by `criterion_id:` field** (NOT by `file:` — that was the prior
model). Issues whose `state` is already in
...
orchestrator does it inline rather than via a separate script. Each cluster
covers ONE `criterion_id` and AT MOST `common/config.yml revise.edit_cap`
...
`cluster_id` (`R<round>-CC-<nnn>`). Output is held in `state.yml` as
`revise_clusters:`:
