#!/usr/bin/env bash
# check-architecture-index.sh — stub for CR-PRD-S* (delivery-2 amendment; full implementation deferred to delivery-3)
# §12.4 contract: stdout = JSON array of issues; exit 0 = pass, 1 = issues, 2 = error
set -euo pipefail
TARGET="${1:?usage: check-architecture-index.sh <target-dir>}"
