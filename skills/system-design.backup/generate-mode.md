...
6a. **Mechanical dependency-layering check** — after every module's Deps (direct) cell is populated, enumerate each `(caller, callee)` pair. For each pair, read the Dependency Layering table and verify the callee's layer is ≤ the caller's layer (same-layer allowed only if documented as a cross-cutting note). Any reverse-layer import is a blocker and must be resolved by either (a) extracting a consumer-side interface into a lower layer, (b) moving the callee to a lower layer, or (c) adding a documented cross-cutting exemption to Dependency Layering. Do not proceed to Step 7 with unresolved reverse imports.
...
7a. **API/module endpoint consistency check** — collect every HTTP method+path mentioned in any module's API Surface table and every endpoint defined in `api/API-*.md`. Produce a diff: (a) module-referenced endpoints missing from API files → open, resolve by adding the endpoint to the right API file; (b) API-defined endpoints absent from any module's API Surface → open, resolve by attaching the endpoint to its owning module. Both sides MUST be in sync before Step 8.
8. **Generate README.md** — aggregate index with all sections
8a. **Implementation Conventions enumeration check** — list every file under the PRD's `architecture/` directory (`ls {PRD path}/architecture/*.md`). For each file, verify that README's Implementation Conventions table has at least one row whose translation sources include that file's topic. Silent omission of a PRD convention file is a blocker — either add the translation row, or add a one-line `N/A — {reason}` note in Implementation Conventions. Both Bidirectional sync with Module Index Deps and the PRD `architecture/*.md` enumeration MUST be complete before Step 9.
8b. **Module Interaction Protocols sync** — enumerate every `(caller, callee)` pair from Module Index Deps and verify each has a corresponding row in Module Interaction Protocols. Missing rows are added before Step 9; extras (rows without a backing Deps pair) are either removed or justified via a cross-cutting note.
9. **Write files** — write all generated files to disk (not yet committed)
9a. **Structural lint pass** — run every check defined in `structural-lint.md` (per-file L1..L5, cross-file X1..X8) against the freshly written directory. Create `.reviews/` if absent; write findings to `.reviews/LINT-{timestamp}.md`. Fix every failure in place via `MultiEdit` before proceeding: placeholder `...` JSON, missing per-endpoint blocks, unfilled Boundary Enforcement columns, dangling endpoint-literal references, PRD architecture/analytics coverage gaps, reverse-layer imports, and single-source-of-truth violations. Re-run the lint until it reports zero failures, then rename the LINT file to `.applied.md`. **Do not proceed to Step 10 with unresolved lint failures** — the semantic self-review is not the place to catch mechanical gaps, and letting them through guarantees they will dominate the subsequent `--review` cycles. (See "Relationship of 9a to earlier checkpoints" below for how 9a differs from 6a/7a/8a/8b.)
10. **Self-review** — read each written file against the Design Review checklist (see `design-review-checklist.md`), fix issues directly in files using the Self-Review Flow defined there. Skip dimensions already validated by Step 9a's structural lint (e.g. per-endpoint API completeness, Boundary Enforcement column fill) unless the lint output flagged them as unresolved. Focus attention on semantic dimensions that structural lint cannot evaluate: responsibility scoping, error-handling depth, NFR decomposition correctness, risk coverage, testability. **Analytics Coverage enumeration**: do not pass this dimension by merely confirming the section exists — load every PRD feature file's `## Analytics` block, count the events, and match against Analytics Coverage row count. Any delta is a finding (and also a structural-lint X4 failure — if structural lint was run cleanly, this delta should be zero by construction).
...
### Relationship of Step 9a to earlier checkpoints (6a / 7a / 8a / 8b)

Those four checkpoints run during interactive refinement and file generation, when fixing a gap is cheap (the agent is still producing the module or README section). Step 9a is the **post-write backstop** — it catches regressions introduced by later steps (e.g. Step 8b adds a protocol row that refers to a module deleted in Step 6, or a README edit in Step 8 orphans an endpoint). The checks overlap deliberately: interactive checks fail fast during generation; Step 9a ensures the final written state is consistent. Never skip Step 9a on the assumption that 6a-8b "already covered it" — interactive checks run before all writes settle.
...
- **Step 6 → 6a:** All module specs generated; each module has interface definition, responsibility, data model (if applicable), Testing section (if applicable — skip for trivial S-complexity modules with no dependencies), and Boundary Enforcement section (if the project has linting/CI infrastructure — skip for trivial S-complexity modules); frontend modules have UI Architecture section complete (component tree, routing, state management, key interactions, performance, a11y implementation, i18n implementation); frontend modules with forms use consistent form patterns per Design System Conventions; HTTP-facing backend modules have API Surface table with every column filled (Method+Path, Auth & Role, Success, Error Codes, Request+Response example links, Constraints); backend modules either satisfy every Backend i18n Implementation trigger-checklist `yes` with a full section OR declare N/A explicitly in Relevant Conventions
- **Step 6a → 7:** All `(caller, callee)` pairs verified against Dependency Layering; any reverse-layer import resolved (lowered via consumer-side interface / callee moved / explicit cross-cutting exemption documented). No unresolved violations remain.
- **Step 7 → 7a:** All API contracts generated (if applicable); every endpoint carries its own Authentication & Permissions, Request table, Request example (populated — no `{}`), Response table, Response example (populated), and Constraints block; dual-surface paths (if any) listed in each endpoint's header; Test Scenarios included for non-trivial APIs
- **Step 7a → 8:** Module API Surface tables and `api/API-*.md` endpoint lists are fully reconciled — every method+path appears in both sides, with no orphan endpoints on either
- **Step 8 → 8a:** README generated with Module Index, Dependency Layering, Module Interaction Protocols, NFR Allocation, Feature-Module Mapping, Analytics Coverage, Test Strategy (including Shared Test Fakes Inventory for fakes used by ≥2 modules), and Implementation Conventions
- **Step 8a → 8b:** Every PRD `architecture/*.md` file has at least one corresponding row in README's Implementation Conventions (or an explicit `N/A — {reason}` note); no PRD convention file silently dropped
- **Step 8b → 9:** Module Index Deps column and Module Interaction Protocols are bidirectionally in sync — every dep pair has a matching protocol row (or a documented cross-cutting note), and every protocol row maps back to a declared dep
- **Step 9 → 9a:** All files written to disk; no partial files; cross-reference paths resolve (relative links to PRD files and between design files)
- **Step 9a → 10:** Structural lint (`structural-lint.md` checks L1..L5 + X1..X8) reports zero failures; every flagged gap has been fixed in place via `MultiEdit`; re-run confirms clean
