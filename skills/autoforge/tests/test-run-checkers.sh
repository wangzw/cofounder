...
# on default-mode output (which has a `FOUND …:` banner before the JSON
# document) and crashed at JSON parse time.
#
# Each scenario invokes $SCRIPT once and captures stdout + stderr via a
# temp file, then asserts both: rc, stdout-parses, banner-on-stderr.
src_clean=$(mktempdir)
stderr_file=$(mktemp)

start_test "--json-only PASS: single run captures stdout + stderr"
out_stdout=$("$SCRIPT" "$plan" --source-root "$src_clean" --json-only 2>"$stderr_file") && rc=0 || rc=$?
...
start_test "--json-only FOUND: single run captures stdout + stderr"
out_stdout=$("$SCRIPT" "$plan" --source-root "$src2" --json-only 2>"$stderr_file") && rc=0 || rc=$?
out_stderr=$(cat "$stderr_file")
assert_exit_code 1 "$rc"
