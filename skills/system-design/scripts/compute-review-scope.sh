...
#
# Usage:
#   scripts/compute-review-scope.sh <design-dir> <round> [--full]
#
# Output:
#   <design-dir>/.review/round-<N>/review-scope.yml
#
# Decision tree:
...
if [[ ${#ARGS[@]} -ne 2 ]]; then
  echo "Usage: $0 <design-dir> <round> [--full]" >&2
  exit 2
fi
...
export DESIGN_DIR ROUND SCRIPT_DIR FULL_FLAG
...
from sd_lint import ROUND_RE, fail_with_script_error  # type: ignore
...
if not os.path.isfile(CURRENT_MANIFEST):
    fail_with_script_error(
        ...
        f"(run snapshot-leaves.sh {DESIGN_DIR} {ROUND} first)"
    )
...
def find_prior_manifest():
    ...
    review_dir = os.path.join(DESIGN_DIR, ".review")
