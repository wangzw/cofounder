...
#     has run commit-delivery.sh (system-design-delivery-<N>-<slug> tag).
...
if not force:
    try:
        proc = subprocess.run(
            ["git", "-C", prd_root, "tag", "--list", f"system-design-delivery-{current_delivery}-*"],
            capture_output=True, text=True, check=False,
        )
        tags = (proc.stdout or "").strip()
        if not tags:
            sys.stderr.write(
                f"warn: no git tag matches 'system-design-delivery-{current_delivery}-*' — the "
