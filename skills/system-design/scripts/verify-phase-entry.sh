...
case "$PHASE" in
  ...
  generate-fresh)
    # Design content leaves should NOT exist — avoid overwriting an existing design bundle.
    found=""
    if [ -f "$PRD_ROOT/README.md" ]; then found="README.md"; fi
    if [ -d "$PRD_ROOT/modules" ] && [ -n "$(ls -A "$PRD_ROOT/modules" 2>/dev/null)" ]; then
      found="${found:+$found, }modules/"
    fi
    if [ -d "$PRD_ROOT/api" ] && [ -n "$(ls -A "$PRD_ROOT/api" 2>/dev/null)" ]; then
      found="${found:+$found, }api/"
    fi
    if [ -n "$found" ]; then
      echo "REFUSE entering generate-fresh phase: existing design content found ($found)"
      echo "  use --evolve to update an existing design instead"
      exit 1
    fi
    echo "OK generate-fresh entry verified — no existing design content"
    exit 0
    ;;
