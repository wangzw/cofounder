...
Supported placeholders in skeleton files:
  system-design        The name of the skill
  docs/raw/design/     The artifact output root directory
  0.1.0     The skill version (e.g. 1.0.0)
  Use when the user needs to create system design documents from a PRD or requirements, perform module decomposition, define interfaces and data models, or review existing designs. The skill description (must start with "Use when")
...
# producing a target skill with literal `system-design` strings in its SKILL.md
