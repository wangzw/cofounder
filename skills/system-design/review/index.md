...
```bash
scripts/snapshot-leaves.sh <design-dir> <current_round>
```

Writes `<design-dir>/.review/round-<N>/leaves-manifest.yml` — a sha256 of
every design leaf (README, modules/M-*.md, api/API-*.md, architecture). This
...
```bash
scripts/compute-review-scope.sh <design-dir> <current_round> [--full]
```

Writes `<design-dir>/.review/round-<N>/review-scope.yml`. Decides whether the
...
Read `<design-dir>/.review/round-<N>/review-scope.yml`. Its `category_clusters:`
...
- `trace_id: R<round>-V-<NNN>` (counter monotonic across all reviewer dispatches
  this round)
- The category name (e.g. `module-boundary`) and the cluster's CR-ID list
- The leaves list (absolute paths from artifact root)
- The design bundle root path (the reviewer reads leaves on demand)
...
- Standard reviewer inputs unchanged: `summary.yml` path, `review-scope.yml` path,
  writer self-review files at `<design-dir>/.review/round-<N>/self-reviews/`
