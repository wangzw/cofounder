...
4. Apply every criterion in `common/review-criteria.md` whose
   ...
   CR-META-adversarial). Do not apply `checker_type: script` criteria —
   those were already enforced by `scripts/run-checkers.sh` (which
   dispatches every per-artifact `check-*.sh`) before you were dispatched.
