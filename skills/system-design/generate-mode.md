...
10. **Self-review** — read each written file against the Design Review checklist (see `design-review-checklist.md`), fix issues directly in files using the Self-Review Flow defined there. **Analytics Coverage enumeration**: do not pass this dimension by merely confirming the section exists — load every PRD feature file's `## Analytics` block, count the events, and match against Analytics Coverage row count. Any delta is a finding.
