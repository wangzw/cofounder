...
( cd "$FIXTURE" && git tag --list | grep -q "^system-design-delivery-1" )
...
( cd "$FIXTURE" && git tag --list ) | grep -q "^system-design-delivery-5" && _record_pass || _record_fail "tag system-design-delivery-5 not found"
teardown_fixture
...
echo "$tag" | grep -qE "^system-design-delivery-2-cleanup" && _record_pass || _record_fail "expected tag prefix 'system-design-delivery-2-cleanup', got '$tag'"
teardown_fixture

test_case "strips redundant '<skill>-delivery-N' prefix from summary"
setup_git_repo
stage_pending_change 3
run_command "$CHECK" "$FIXTURE" "3" "system-design-delivery-3: cleanup"
tag=$(cd "$FIXTURE" && git tag --list | head -1)
echo "$tag" | grep -qE "^system-design-delivery-3-cleanup$" && _record_pass || _record_fail "expected tag 'system-design-delivery-3-cleanup', got '$tag'"
teardown_fixture
