...
| `{{TRIGGERED_BY}}` | Comma-separated filenames, e.g. `REVIEW-001.md, LINT-001.md` | The REVIEW-*.md and/or LINT-*.md files consumed by this revise pass. Use bare filenames (no path prefix). If no REVIEW or LINT file was consumed (interactive-only pass), write `interactive`. |
...
| **Revision ID** | REV-002 |
| **Date** | 2026-05-14 |
| **Triggered by** | REVIEW-001.md, LINT-001.md |
...
Extracted cache invalidation logic from M-004 into a new M-007 module to eliminate the circular dependency flagged in REVIEW-001. Updated API-002's response schema to include a `cache_hit` boolean field per LINT-001 L2 finding (placeholder JSON removed from example block).

### Verification

Lint gate: PASS (0 failures)
Cross-reviewer status: Addressed 4/4 open findings from REVIEW-001.md
```
