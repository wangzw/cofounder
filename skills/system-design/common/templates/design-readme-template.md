...
### Machine-Readable Feature-Module Map

<!-- COMMENTARY: Alongside the human-readable Feature-Module Mapping matrix above, the
     writer MUST also generate a machine-readable YAML file at
     `{design-dir}/feature-module-map.yml`. This file is the input to the
     `/system-design delta F-NNN` command and to autoforge's feature-scope mode.
     One entry per PRD feature. -->

Format:

```yaml
# feature-module-map.yml — auto-generated by system-design global mode
# Do NOT edit manually. Regenerate via /system-design <prd-dir>.
...
<!-- COMMENTARY:
     - `writes`: modules where this feature's implementation modifies code or data.
       Maps to ✦ cells in the Feature-Module Mapping matrix.
     - `reads`: modules where this feature relies on read-only data/exports.
       Maps to △ cells in the Feature-Module Mapping matrix.
     - The map is the single source of truth for delta analysis and regression scoping.
     - If a feature has been deprecated (tombstoned), omit it from the map. -->
