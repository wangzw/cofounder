...
## Role: per-criterion-cluster-reviser for system-design
...
3. **Affected leaves** — absolute paths (from artifact root) of every leaf
   touched by at least one of your issues (e.g.
   `modules/M-001-auth.md`, `api/API-002-public.md`, `README.md`). Read
   each leaf at processing time, in order — NOT in advance. Reading all
   leaves up front bloats sub-agent cache for no reason.
...
- **Do not** Write, Edit, or NotebookEdit any file under `~/.claude/skills/`
  ...
  (e.g. "update CR-SD-XX in review-criteria.md"), that is an out-of-scope
...
```
OK trace_id=R3-R-001 role=reviser cluster_id=R3-CC-001 criterion_id=CR-SD-DESIGN01 linked_issues=I-007,I-019,I-024
```
