...
- For issues with `blocker_scope: global-conflict` that have been escalated to the reviser
  by the cross-reviewer: **do NOT apply a fix in this dispatch**. The per-leaf reviser scope
  is structurally incapable of resolving cross-artifact conflicts — the reviser has the
  same single-leaf scope as the writer that originally punted with `blocker_scope:
  global-conflict`. Instead:
