...
- During Bootstrap Precheck, orchestrator MUST write
  `skill_forge_dir: <absolute path to the skill-forge directory>` to
  `<target>/.review/state.yml` so downstream sub-agents can locate skill-forge's own scripts.
...
4. Copy Snippet C verbatim from `<skill-forge>/common/snippets.md`; do not paraphrase or shorten.
